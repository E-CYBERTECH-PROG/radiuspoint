<x-sidebar-layout title="Hotspot Service Sales">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Hotspot Service Sales</h1>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Successful transactions matched to your Hotspot packages by name.</p>
    </div>
    ...
    <form method="GET" class="mb-6 flex flex-col sm:flex-row gap-3">
        ...
        @if(request()->hasAny(['from', 'to']))
            <a href="{{ route('reports.hotspot-sales') }}" class="text-sm text-gray-500 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 self-center">Clear</a>
        @endif
    </form>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center text-gray-400">
                                ...
                                <p class="text-xs tracking-widest uppercase">No hotspot service sales matched yet.</p>
                                <p class="text-xs text-gray-400 mt-2 normal-case">Only transactions whose package name exactly matches a current Hotspot plan name are shown.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
