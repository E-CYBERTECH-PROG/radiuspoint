<x-sidebar-layout title="Leads">
    <div class="mb-4 d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-end gap-3">
        ...
        <button type="button" class="btn btn-primary" data-bs-toggle="offcanvas" data-bs-target="#rp-add-lead">
            <i class="ti ti-plus icon"></i> New Lead
        </button>
    </div>
    ...
    <form method="GET" class="mb-4 d-flex flex-column flex-sm-row gap-2">
        ...
        @if(request()->hasAny(['search', 'status']))
            <a href="{{ route('leads.index') }}" class="btn btn-link align-self-center">Clear</a>
        @endif
    </form>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="text-center text-muted py-5">
                                <i class="ti ti-user-plus icon icon-lg mb-2 d-block"></i>
                                <p class="text-uppercase small mb-0">No leads captured yet.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
