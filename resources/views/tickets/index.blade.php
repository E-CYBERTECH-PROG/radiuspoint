                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-900/50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <form action="{{ route('tickets.update-status', $ticket) }}" method="POST" class="inline-block">
                                        @csrf @method('PATCH')
                                        <select name="status" onchange="this.form.submit()" class="text-[10px] tracking-wide uppercase font-bold rounded px-2 py-1 border outline-none cursor-pointer {{ $ticket->status == 'resolved' || $ticket->status == 'closed' ? 'bg-green-50 text-green-700 border-green-200 dark:bg-green-900/20 dark:text-green-400 dark:border-green-900/50' : ($ticket->status == 'in_progress' ? 'bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-900/20 dark:text-blue-400 dark:border-blue-900/50' : 'bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-900/20 dark:text-amber-400 dark:border-amber-900/50') }}">
                                ...
                                <td class="px-6 py-4 text-right relative">
                                    <div class="flex items-center justify-end gap-3">
                                        ...
                                        <form action="{{ route('tickets.destroy', $ticket) }}" method="POST" onsubmit="return confirm('Remove this ticket?')">
                                            @csrf @method('DELETE')
